...
def save_commission_to_db(current_stats: dict) -> None:
    """Save commission data to commission.db SQLite database"""
    try:
        # Save commission data to commission.db
        commission_old = current_stats.get("commission_old", 0)
        commission_change = current_stats.get("commission_change", 0)
        commission_current = current_stats.get("commission_current", 0)
        week_change_in_commission = current_stats.get("week_change_in_commission", 0)
        ...
        if existing_record:
            ...
            cursor.execute('''
                ...
                SET timestamp = CURRENT_TIMESTAMP,
                    ...
                    setting_status = ?
                WHERE user_email = ?
            ''', (commission_old, commission_change, commission_current, week_change_in_commission, 1, user_email))
            logger.debug(f"Updated commission data for email: {user_email}")
        else:
            # Insert new record
            cursor.execute('''
                INSERT INTO commission_data (commission_old, commission_change, commission_current, week_change_in_commission, setting_status, user_email)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (commission_old, commission_change, commission_current, week_change_in_commission, 1, user_email))
